...
public class HttpRequestServerHandler extends SimpleChannelInboundHandler<HttpObject> {
	...
	public HttpRequestServerHandler(HttpConfig config) {
		this.config = config;
	}
	...
	protected void messageReceived(ChannelHandlerContext ctx, HttpObject msg) throws Exception {
		...
		try {
			if (msg instanceof HttpRequest) {
				...
				/**
				 * 上传情况 比较特殊
				 * //&&"multipart/form-data".equals(RequestParser.getContentType(request))
				 */
				if (request.method() == HttpMethod.POST && HttpPostRequestDecoder.isMultipart(request)) {
					// 4.x-此方法执行 意味着已经提交结束 并且获取剩下的参数
					requestInfo = new RequestHttpContentParser().parse(request);
					// **************************************
					// 5-创建模板中会使用的必要的类 FullHttpResponse FullHttpRequest
					// ChannelHandlerContext
					FullHttpResponse response = new DefaultFullHttpResponse(HttpVersion.HTTP_1_1,
							HttpResponseStatus.OK);
					response.headers().set(HttpHeaderNames.CONTENT_TYPE,
							info.getResultType().getContentTypeString(this.config.getCharSet().name()));
						...
						for (int i = 0; i < parameterNames.length; i++) {
							if (paramMap.containsKey(parameterNames[i])) {
								if (!info.getClazzs()[i].isArray()) {
									Object object = TypeUtils.cast(paramMap.get(parameterNames[i]), info.getClazzs()[i],
											ParserConfig.getGlobalInstance());
									params[i] = object;
								} else if (info.getClazzs()[i] == String.class) {
									...
									sBuilder.append("]");
									Object object = JSONObject.toJavaObject(JSON.parseArray(sBuilder.toString()),
											info.getClazzs()[i]);
									params[i] = object;
								}
							} else {
					...
					if (info.isSpringBean()) {
						CallCapital callCapital = (CallAbstract) SpringBeanUtil.getApplicationContext()
								.getBean(info.getBeanName());
					...
					if (!hasCtx) {
						...
						if (result != null) {
							...
							// this.httpServer.setDataByteTrans(new
							// NormalDataByteTrans());
							// }
							...
							if (paramMap.containsKey(parameterNames[i])) {
								if (!info.getClazzs()[i].isArray()) {
									Object object = TypeUtils.cast(paramMap.get(parameterNames[i]), info.getClazzs()[i],
											ParserConfig.getGlobalInstance());
									params[i] = object;
								} else if (info.getClazzs()[i].isArray()) {
									String arraystring = (String) paramMap.get(parameterNames[i]);
									StringTokenizer stringTokenizer = new StringTokenizer(arraystring, ",");
									StringBuilder sBuilder = new StringBuilder("[");
									while (stringTokenizer.hasMoreTokens()) {
				...
				ResponseHelper.sendError(ctx, HttpResponseStatus.NOT_ACCEPTABLE);
			}
		} finally {
	...
	private static void sendError(ChannelHandlerContext ctx, HttpResponseStatus responseState) {
		FullHttpResponse response = new DefaultFullHttpResponse(HttpVersion.HTTP_1_1, responseState, Unpooled
				.copiedBuffer("Failure: " + responseState.toString() + System.lineSeparator(), CharsetUtil.UTF_8));
